...
**Окно спит, когда ничего не происходит.** Начиная с 1.9.3 UI не перерисовывается по таймеру: потоки, которые меняют показываемое, — хуки, трей, расписание, воспроизведение — будят его сами. Устоявшееся окно стоит около 0,2 % одного ядра и четверть кадра в секунду против 1,6 % и 8,8 кадра раньше, а горячая клавиша, нажатая когда впереди чужое приложение, теперь доходит до окна сразу, а не на следующем тике.
...
У каждого шага и каждого условия с картинкой есть собственная **область поиска**: весь экран, активное окно, заданный прямоугольник, рядом с прошлым совпадением или относительно другой картинки. Это самое полезное поле выпуска, и сильнее всего это проявляется в ожидании, потому что ожидание повторяет поиск: `Ждать картинку` смотрит заново каждые 120 мс, так что тридцать секунд ожидания по всему экрану — это 250 полноэкранных поисков, а замер от начала до конца даёт 1.8–2.3 с процессора на десять секунд ожидания. С 1.9.4 **Проверить макрос** об этом говорит и называет число просмотров; ограничить область — это один выпадающий список, а таблица ниже показывает, что это даёт. Замерено на столе 2560×1440 через `--selftest vision`, один шаг, ищущий шаблон 64×64:
...
**Таймлайн каждого прогона.** Каждый шаг оставляет отметку — когда начался и каким шагом был, — и история запусков показывает их под прогоном в виде `mm:ss.mmm` с длительностью каждого шага. Медленные шаги подсвечены, а шаг, на котором прогон оборвался, — другим цветом. Хранятся последние двадцать тысяч отметок, так что у макроса, упавшего на девятом часу, остаются именно те минуты, которые важны.

**Запись экрана.** Необязательная, по умолчанию выключена: включите — и нажатие «Запись» дополнительно пишет видео H.264 в `recordings/` столько, сколько идёт запись макроса. Идёт через Media Foundation, поэтому кодирует видеокарта, если она есть, — NVIDIA, AMD или Intel одинаково, — и процессор, если её нет, и в обоих случаях без сторонних зависимостей. Замерено на столе 2560×1440: 30 кадров в секунду без пропусков. Индекс файла записывается при остановке, поэтому видео от принудительно завершённого процесса не откроется.

**Экранный дисплей.** То же окно может нести поверх всего сводку: что делает макрос (прогон, шаг, из какого файла), что ему пришлось обойти (fallback'и, блоки восстановления, повторы OCR, промахнувшиеся шаги), состояние транспорта и отзывчивость целевого окна. Пять пресетов и четыре угла, по умолчанию выключен. Продолжает работать, когда главное окно свёрнуто в трей, — в этом и смысл: смотрят обычно на игру.

Блок отзывчивости озаглавлен `RESPONSE`, а не `GAME`, и это намеренно: там скорость ответа целевого окна на сообщения, а не скорость отрисовки. Настоящие тайминги кадров требуют ETW-сессии к провайдерам DXGI и прав администратора.

**«Показывать, куда смотрит скрипт»** кладёт поверх всего прозрачное окно, сквозь которое проходят клики. Пока идёт скрипт, оно рисует область поиска синим, совпадение с оценкой — зелёным или красным, прямоугольник чтения текста — янтарным, найденный элемент интерфейса — фиолетовым. Оверлей Wayland отмечает крестом и отдельные точки: бирюзовым — куда разрешилась цель, розовым — пиксель условия по цвету, белым — куда пришёлся клик.
...
> *С 1.9.1* справочник написан на всех шести языках интерфейса — английском,
> русском, украинском, португальском, испанском и китайском — и следует тому,
> на который выставлен интерфейс. Как и любую другую строку программы, его можно
> переопределить через `lang/<код>.json` без пересборки.
...
```bash
./install.sh          # любой дистрибутив: определяет систему и окружение, ставит зависимости, собирает, устанавливает
clickwork --doctor    # что умеет эта машина
```

На Hyprland горячие клавиши регистрируются ещё и как бинды композитора, поэтому F6 начинает запись даже там, где браузер иначе перехватил бы клавишу.
...
А **[PLATFORMS_RU.md](PLATFORMS_RU.md)** — это сравнение: Windows против Linux возможность за возможностью, какой из четырёх пакетов Linux брать и до какого старого дистрибутива каждый достаёт, и измеренная таблица девяти композиторов — что работает на Hyprland, KDE, sway, niri, river, Wayfire, labwc, COSMIC и GNOME, а что нет.
...
|---|---|
| **Windows или Linux под Wayland** | На Linux поддерживаемый композитор — Hyprland, он отвечает на всё. Другие композиторы на wlroots дают воспроизведение, захват, OCR и поиск окон, но не геометрию окон и не изоляцию рабочих столов. У KDE своя дорога ко всему этому, и для неё программа должна быть установлена. GNOME умеет только запись. Записи нужен доступ на чтение `/dev/input`. См. [LINUX_RU.md](LINUX_RU.md) |
...
Проект полностью **открыт**, и в этом вся история про безопасность: то, что вы
запускаете, собрано из кода в этом репозитории, и вы можете собрать его сами.
...
Чтобы убедиться, что скачанный `.exe` — тот самый, который опубликован, выполните в
PowerShell и сравните результат с хешем на странице релиза:

```powershell
Get-FileHash -Algorithm SHA256 .\имя_файла.exe
```

</details>

<details>
<summary><b>🔎 Что программе нужно и зачем</b></summary>

<br>

| Возможность | Для чего |
|---|---|
| Глобальные хуки клавиатуры и мыши | Запись ввода и горячие клавиши, работающие поверх чужого окна |
| Синтетический ввод (`SendInput`) | Воспроизведение |
| Снимок экрана | Поиск картинки, распознавание текста, отладочный оверлей |
| UI Automation | Поиск элемента по имени, а не по координате |
| Буфер обмена | Текстовый расширитель и шаги `Копировать`/`Вставить` |
| Привилегия завершения работы | Только когда выбрано действие питания в конце прогона |

Никуда ничего не отправляется. Сетевого кода в программе нет вообще — ни телеметрии,
ни проверки обновлений, ни отправки отчётов о сбоях.
